...
void mqttTask(void* parameter) {
	...
	while (1) {
		...
		if (((xTaskGetTickCount() - lastTime)/xPortGetTickRateHz()) >= 60) {
			...
			int perillaSensorValue = processingData(580, 840, adc1_get_raw(ADC1_CHANNEL_7));  //840 - 580
			
			processingData(320, 1023, lemongrassSensorValue);
			processingData(320, 1023, lemongrassSensorValue);
			processingData(320, 1023, lemongrassSensorValue);
			processingData(320, 1023, lemongrassSensorValue);
			...
			minPassed++;
			snprintf(mqttString, 120, "Lemon Grass: %d%% | Chinese Celery: %d%% | Rosemary: %d%% | Perilla: %d%%", 
				lemongrassPC, chineseceleryPC, rosemaryPC, perillaPC);
			Serial.println("####################################################");
...
int processingData(int min, int max, int data) {
